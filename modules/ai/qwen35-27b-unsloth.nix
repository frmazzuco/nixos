...
let
  ...
  qwen35_27BChat = pkgs.writeShellScriptBin "qwen35-27b-chat" ''
    ...
    MODEL_FILE="''${QWEN35_27B_FILE:-${qwen35_27BModelFile}}"
    if [ ! -f "$MODEL_FILE" ]; then
      echo "Modelo nao encontrado em $MODEL_FILE" >&2
      echo "Rode: qwen35-27b-download" >&2
      exit 1
    fi

    export CUDA_VISIBLE_DEVICES="''${CUDA_VISIBLE_DEVICES:-0}"

    PROFILE="''${QWEN35_27B_PROFILE:-instruct-fast}"
    case "$PROFILE" in
      speed|instruct-fast)
      ...
      thinking-coding)
        ...
        ;;
      *)
        echo "Perfil QWEN35_27B_PROFILE invalido: $PROFILE" >&2
    ...
    exec ${llamaCppCuda}/bin/llama-cli \
      ...
      --reasoning-format "''${QWEN35_27B_REASONING_FORMAT:-$DEFAULT_REASONING_FORMAT}" \
      --reasoning-budget "''${QWEN35_27B_REASONING_BUDGET:-$DEFAULT_REASONING_BUDGET}" \
      --temp "''${QWEN35_27B_TEMP:-$DEFAULT_TEMP}" \
      --top-p "''${QWEN35_27B_TOP_P:-$DEFAULT_TOP_P}" \
      --top-k "''${QWEN35_27B_TOP_K:-$DEFAULT_TOP_K}" \
      --min-p "''${QWEN35_27B_MIN_P:-$DEFAULT_MIN_P}" \
      --presence-penalty "''${QWEN35_27B_PRESENCE_PENALTY:-$DEFAULT_PRESENCE_PENALTY}" \
      --repeat-penalty "''${QWEN35_27B_REPEAT_PENALTY:-$DEFAULT_REPEAT_PENALTY}" \
      --ctx-size "''${QWEN35_27B_CTX:-8192}" \
      --batch-size "''${QWEN35_27B_BATCH:-1024}" \
      --ubatch-size "''${QWEN35_27B_UBATCH:-256}" \
      --threads "''${QWEN35_27B_THREADS:-12}" \
      --threads-batch "''${QWEN35_27B_THREADS_BATCH:-12}" \
      --flash-attn on \
      --parallel "''${QWEN35_27B_PARALLEL:-1}" \
      --fit "''${QWEN35_27B_FIT:-on}" \
      --fit-target "''${QWEN35_27B_FIT_TARGET:-256}" \
      --fit-ctx "''${QWEN35_27B_FIT_CTX:-4096}" \
      --cache-type-k "''${QWEN35_27B_CACHE_K:-q4_0}" \
      --cache-type-v "''${QWEN35_27B_CACHE_V:-q4_0}" \
      "$@"
  '';

  qwen35_27BServer = pkgs.writeShellScriptBin "qwen35-27b-server" ''
    set -euo pipefail

...
reject_fixed 'requires = [ "gemma4-26b-server.service" ];' "$module_file"
require_fixed 'after = [ "gemma4-26b-server.service" ];' "$module_file"
require_fixed 'AMBIENT_ASSISTANT_PROVIDER_KIND=openai-compat' "$module_file"
require_fixed 'ports = import ../ai/ports.nix;' "$module_file"
require_fixed 'AMBIENT_ASSISTANT_PROVIDER_BASE_URL=http://127.0.0.1:${toString ports."gemma4-26b"}/v1' "$module_file"
require_fixed '"gemma4-26b" = 18083;' "$ports_file"
reject_fixed 'AMBIENT_ASSISTANT_PROVIDER_BASE_URL=http://127.0.0.1:18083/v1' "$module_file"
require_fixed 'AMBIENT_ASSISTANT_MODEL=gemma-4-26B-A4B-it-UD-IQ4_XS.gguf' "$module_file"
...
reject_fixed 'AMBIENT_ASSISTANT_MODEL=gemma-4-e4b-it-Q8_0.gguf' "$module_file"
